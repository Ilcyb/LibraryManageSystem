...
def lending_history():
    ...
    for i in range(len(lending_infos)):
        ...
        lendinfo_json['timeout'] = lending_infos[i].timeout
        if lending_infos[i].timeout:
            lendinfo_json['timeout_time'] = (lending_infos[i].return_time - lending_infos[i].expected_return_time).days
